...
document.getElementById("checkkw").onclick = function (e) {
  ...
  if (!isNaN(kwNum) && kwNum > 0) {
    if (kwNum <= 50) {
      money = kwNum * 500;
      result = `<h1 class="text-green-600">Họ tên ${nameKw} - Tổng tiền: ${money.toLocaleString(
        "vi-VN"
      )} đồng</h1>`;
      ...
      money =
        50 * 500 + 50 * 650 + 100 * 850 + 150 * 1100 + (kwNum - 350) * 1300;
      result = `<h1 class="text-green-600">Họ tên ${nameKw} - Tổng tiền: ${money.toLocaleString(
        "vi-VN"
      )} đồng</h1>`;
      showMoney.innerHTML = result;
...
document.getElementById("showTax").onclick = function (e) {
  ...
  if (totalSalary > 0) {
    showResult3.innerHTML = `<h1 class="text-green-600">Họ tên ${nameBT3} - Tổng tiền thuế: ${tax.toLocaleString(
      "vi-VN"
    )} đồng</h1>`;
  }
...
typeCustomer.onchange = function () {
  if (typeCustomer.value === "DoanhNghiep") {
...
document.getElementById("MoneyChanel").onclick = function (e) {
  ...
  let totalMoney = 0;
  if (typeCustomer === "NhaDan") {
    totalMoney = 4.5 + 20.5 + numChanelHigh * 7.5;
  } else if (typeCustomer === "DoanhNghiep") {
    let basicFee = 75;
    if (numConnect > 10) {
      basicFee += (numConnect - 10) * 5;
    }
    totalMoney = 15 + basicFee + numChanelHigh * 50;
  }

  showMoneyChanel.innerHTML = `<h1 class="text-green-600">Mã Khách hàng ${idCustomer} - Tổng tiền cáp: $ ${totalMoney.toFixed(
    2
  )}</h1>`;
};
